...
import com.company.model.RegistoGrausProficiencia;
...
public class EspecificarCompetenciaTecnicaController {
    ...
    private RegistoGrausProficiencia rgp;
     ...
     * @param grausProficiencia graus de Proficiencia que esta Competência Técnica irá ter
    ...
    public boolean novoGrauProficiencia (String valor, String designacao) throws Exception {
          try {
            GrauProficiencia gp = compTecnica.getGrausProficiencia().novoGrauProficiencia(valor, designacao, compTecnica);
            if (compTecnica.getGrausProficiencia().validaGrauProficiencia(gp)) 
                return compTecnica.getGrausProficiencia().registaGrauProficiencia(gp);
        } catch (Exception ex) {
            throw new Exception(ex);
        }
        return false;
     ...
     */
    public List<AreaAtividade> getAreasAtividade() throws SQLException {
     ...
     */
    public List<CompetenciaTecnica> getListCompTec() throws SQLException {
     ...
     */
    public ArrayList<String> getListaAreaAtividadeIdDescBreve() throws SQLException {
     ...
     */
    public AreaAtividade getAreaAtividadeById(String string) throws SQLException {
        String trimmed[] = string.trim().split(" ");
        return plat.getLat().getAreaAtividadeById(trimmed[1]);
    }

    public List<GrauProficiencia> getCompTecGrausProf(CompetenciaTecnica selectedItem) throws SQLException {
        return plat.getRct().getCompetenciaTecnicaById(selectedItem.getId()).getListaGrausProficiencia();
    }
