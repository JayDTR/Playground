...
public class AdicionarTarefaUI implements Initializable {
    ...
    private EspecificarTarefaController tarCtrl;
        ...
        try {
            if (tarCtrl.novaTarefa(txtNovaTarefaRef.getText().trim(), txtNovaTarefaDesign.getText().trim(), txtNovaTarefaDescInformal.getText().trim(),
                    txtNovaTarefaDescTecnica.getText().trim(), Integer.parseInt(txtNovaTarefaDuracao.getText().trim()),
                                                                                       //     CatTar-10               10 : desc
                    Double.parseDouble(txtNovaTarefaCusto.getText().trim()), tarCtrl.getCategoriaTarefa("CatTar-" + cmbNovaTarefaCatTarefa.getValue().split(" ")[0]), tarCtrl.getColaborador())) {
    ...
    void updateCatTarCMB() throws SQLException {
    ...
    void associarParentUI(TabTarefasUI tabTarefasUI,EspecificarTarefaController tarCtrl) {
...
//                Alert alerta = AlertaUI.criarAlerta(Alert.AlertType.INFORMATION, TITULO_APLICACAO,
//                        "Caracter Inválido Caracteres", "Insira Formato XXXXXX");
//
//                if (alerta.showAndWait().get() == ButtonType.OK) {
//                    event.consume();
//                }
