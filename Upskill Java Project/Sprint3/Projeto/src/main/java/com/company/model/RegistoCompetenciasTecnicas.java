...
import com.company.data.CompetenciaTecnicaDB;
...
public class RegistoCompetenciasTecnicas {
    ...
    private CompetenciaTecnicaDB ctDB;
    ...
    public RegistoCompetenciasTecnicas() {
        ...
        this.ctDB = new CompetenciaTecnicaDB();
    }
     ...
     */
    public CompetenciaTecnica novaCompetenciaTecnica(String id, String descBreve, String descDetalhada, AreaAtividade aa) throws SQLException {
        CompetenciaTecnica ct = new CompetenciaTecnica(id, descBreve, descDetalhada, aa);
        return ctDB.insertCT(ct);
    }
     ...
     */
    public List<CompetenciaTecnica> getListaCompTec() throws SQLException  {
        List<CompetenciaTecnica> tmp = ctDB.getAllCompetenciasTecnicas();

        for (CompetenciaTecnica ct : tmp) {
            this.lcompTec.add(ct);
        }
        return ctDB.getAllCompetenciasTecnicas();
    }
    ...
    /**
     * Devolve uma competência técnica existente na lista de competências
     * técnicas com id igual ao passado por parâmetro.
     *
     * @param strId identificação da competência técnica
     * @return competência técnica, ou null caso o id não exista na lista
     */
    public CompetenciaTecnica getCompetenciaTecnicaById(String strId) throws SQLException {

        for (CompetenciaTecnica ct : ctDB.getAllCompetenciasTecnicas()) {
            if (ct.hasId(strId)) {
                return ct;
            }
        }
    ...
    /**
     * Devolve uma competência técnica existente na lista de competências
     * técnicas com a descrição igual à que é passada por parâmetro.
     *
