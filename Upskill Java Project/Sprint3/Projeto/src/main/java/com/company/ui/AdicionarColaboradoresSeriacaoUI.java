...
public class AdicionarColaboradoresSeriacaoUI implements Initializable {
    ...
    private void actionAddPSeriacao(ActionEvent event) {
        try {
            ...
            if (sCtrl.novaProcessoSeriacao(colabs)) {
                showAACreationSuccess();
                 adicionarSeriacaoUI.updateListSeriacao();
                encerrarNovaSeriacaoColaboradoresUI(event);
            }
        } catch (Exception ex) {
    ...
    void associarParentUI(AdicionarSeriacaoUI adicionarSeriacaoUI, EspecificarSeriacaoController sCtrl) {
