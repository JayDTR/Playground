...
public class AdicionarCandidaturaUI implements Initializable {
    ...
    @FXML
    private void actionAdicionarCandidatura(ActionEvent event) throws SQLException {
        candCtrl = new EfetuarCandidaturaController();
        Freelancer free = candCtrl.getFreelancerByEmail();

        try {
            if (candCtrl.novaCandidatura(tabAnunciosDisponiveisUI.tableViewAnuncio(), Double.parseDouble(txtValorPretendido.getText()),
                    Integer.parseInt(txtNrDias.getText()), txtApr.getText(), txtMotiv.getText(), free)) {
                showAACreationSuccess();
                tabAnunciosDisponiveisUI.updateListCandidaturas();
