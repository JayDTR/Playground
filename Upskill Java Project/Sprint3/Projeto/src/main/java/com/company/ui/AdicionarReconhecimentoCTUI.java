...
public class AdicionarReconhecimentoCTUI implements Initializable {
    ...
    void updateCT() throws SQLException {
    ...
    private void actionFinalFreelancerAdd(ActionEvent event) throws Exception {
        try {
            if (fCtrl.novoReconhecimentoCT(Date.valueOf(reconDate.getValue()), fCtrl.getSelectedGrau(cmbGrauCompTec.getValue().split(" ")[1]+" "+cmbGrauCompTec.getValue().split(" ")[2]))) {
                if (fCtrl.novoFreelancer(adicionarHabilitacoesAcademicasUI.getAddFreelancerUI().getAddFreelancerUiTexts().get(0),
    ...
    @FXML
    private void CompTecSelec(ActionEvent event) throws SQLException {
