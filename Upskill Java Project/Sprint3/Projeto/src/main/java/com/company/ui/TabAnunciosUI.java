...
public class TabAnunciosUI implements Initializable {
    ...
    void updateSeriacaoList() throws SQLException {
        mainSceneUI.getTabSeriacaoUI().updateList();
    }
    ...
    @FXML
    private void clkAnuncio(MouseEvent event) {
    ...
    @FXML
    private void actionSeriar(ActionEvent event) throws SQLException {

        if (getAnuncioSelected().getRegimento().getId() == 3) {
            try {
                List<Colaborador> colabs = new ArrayList<>();
                sCtrl = new EspecificarSeriacaoController();
                sCtrl.setListaCand(getAnuncioSelected());
                if (sCtrl.novaProcessoSeriacaoAuto()) {
